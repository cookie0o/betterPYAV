...
def file_MLCheck(self, current_dir, filepath):
    ...
    clf = jobload((current_dir+"/dep/ML/classifiers/file_data_classifier.pkl"))

    with open((current_dir+"/dep/ML/classifiers/file_data_features.pkl"), 'rb') as f:
        features = load(f)
    ...
    proba = clf.predict_proba([pe_features])[0]

    # Get the probability of the positive class in % (0,)
    percentage = proba[1]

    MLCheck = clf.predict([pe_features])[0]
    
    return MLCheck, percentage